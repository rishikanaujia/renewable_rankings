"""Ownership Consolidation Extractor - AI-powered extraction for market concentration."""
...
class OwnershipConsolidationExtractor(BaseExtractor):
    """Extractor for ownership consolidation parameter."""
    ...
    def _get_extraction_prompt(self, country: str, document_content: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        return f"""Extract ownership consolidation and market concentration data for renewable energy in {country}.

Analyze market share of top 3 owners, HHI (Herfindahl-Hirschman Index), number of significant players, and market competitiveness.
...
Score Guidelines (INVERSE - lower consolidation = higher score):
- 1-2: Extreme monopoly (>80% by single owner)
- 3-4: High consolidation (60-70% by top 3)
- 5-6: Moderate consolidation (40-50% by top 3)
- 7-8: Low consolidation (20-30% by top 3)
- 9-10: Highly fragmented (<10% by top 3)

IMPORTANT: Lower market consolidation = More competitive = HIGHER score (inverse relationship)"""
    ...
    def _parse_llm_response(self, llm_response: str, country: str) -> Dict[str, Any]:
        try:
            ...
            logger.info(f"Parsed ownership consolidation for {country}: score={score}/10, confidence={parsed['confidence']:.2f}")
            return parsed
    ...
    def get_required_documents(self) -> List[str]:
        return ['market_reports', 'ownership_databases', 'industry_statistics']
