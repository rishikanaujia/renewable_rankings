"""Power Market Size Extractor - AI-powered extraction for electricity demand and market size."""
...
class PowerMarketSizeExtractor(BaseExtractor):
    """Extractor for power market size parameter."""
    ...
    def _get_extraction_prompt(self, country: str, document_content: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        return f"""Extract power market size and electricity demand for {country}.

Analyze total electricity consumption (TWh), market size, demand growth, and addressable market for renewable energy.
...
Score Guidelines:
- 1-2: Tiny market (<10 TWh/year), very limited opportunity
- 3-4: Small market (10-50 TWh/year), limited opportunity
- 5-6: Moderate market (50-200 TWh/year), decent opportunity
- 7-8: Large market (200-1000 TWh/year), good opportunity
- 9-10: Massive market (>1000 TWh/year), excellent opportunity"""
    ...
    def _parse_llm_response(self, llm_response: str, country: str) -> Dict[str, Any]:
        try:
            ...
            logger.info(f"Parsed power market size for {country}: score={score}/10, confidence={parsed['confidence']:.2f}")
            return parsed
    ...
    def get_required_documents(self) -> List[str]:
        return ['electricity_statistics', 'market_reports', 'demand_forecasts']
