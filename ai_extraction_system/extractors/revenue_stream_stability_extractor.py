"""Revenue Stream Stability Extractor - AI-powered extraction for revenue mechanism stability."""
...
class RevenueStreamStabilityExtractor(BaseExtractor):
    """Extractor for revenue stream stability parameter."""
    ...
    def _get_extraction_prompt(self, country: str, document_content: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        return f"""Extract revenue stream stability for renewable energy projects in {country}.

Analyze PPA stability, contract enforcement, payment reliability, merchant risk exposure, and revenue predictability.
...
Score Guidelines:
- 1-2: Very unstable, high merchant exposure, unreliable payments
- 3-4: Unstable revenue, significant payment risks
- 5-6: Moderate stability, some merchant exposure
- 7-8: Stable revenue, strong PPA mechanisms
- 9-10: Very stable revenue, government-backed contracts"""
    ...
    def _parse_llm_response(self, llm_response: str, country: str) -> Dict[str, Any]:
        try:
            ...
            logger.info(f"Parsed revenue stream stability for {country}: score={score}/10, confidence={parsed['confidence']:.2f}")
            return parsed
    ...
    def get_required_documents(self) -> List[str]:
        return ['ppa_contracts', 'payment_history', 'market_regulations']
