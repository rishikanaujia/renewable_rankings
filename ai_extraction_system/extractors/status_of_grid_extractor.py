"""Status of Grid Extractor - AI-powered extraction for grid infrastructure quality."""
...
class StatusOfGridExtractor(BaseExtractor):
    """Extractor for status of grid parameter."""
    ...
    def _get_extraction_prompt(self, country: str, document_content: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        return f"""Extract grid infrastructure quality and status for renewable energy in {country}.

Analyze grid reliability, transmission capacity, interconnection capability, curtailment levels, and grid modernization.
...
Score Guidelines:
- 1-2: Very poor grid, high curtailment, unreliable
- 3-4: Poor grid infrastructure, frequent outages
- 5-6: Moderate grid, some curtailment issues
- 7-8: Good grid infrastructure, reliable interconnection
- 9-10: Excellent grid, minimal curtailment, world-class"""
    ...
    def _parse_llm_response(self, llm_response: str, country: str) -> Dict[str, Any]:
        try:
            ...
            logger.info(f"Parsed status of grid for {country}: score={score}/10, confidence={parsed['confidence']:.2f}")
            return parsed
    ...
    def get_required_documents(self) -> List[str]:
        return ['grid_reliability_reports', 'transmission_studies', 'curtailment_data']
