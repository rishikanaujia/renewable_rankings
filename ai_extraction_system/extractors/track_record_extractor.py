"""Track Record Extractor - AI-powered extraction for renewable energy deployment history."""
...
class TrackRecordExtractor(BaseExtractor):
    """Extractor for track record parameter."""
    ...
    def _get_extraction_prompt(self, country: str, document_content: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        return f"""Extract track record for renewable energy deployment in {country}.

Analyze deployment history, installed capacity, growth trends, and project success rates.
...
Respond with JSON containing: value (1-10 score), confidence, justification, quotes, metadata."""
    ...
    def _parse_llm_response(self, llm_response: str, country: str) -> Dict[str, Any]:
        try:
            ...
            logger.info(f"Parsed track record for {country}: score={score}/10, confidence={parsed['confidence']:.2f}")
            return parsed
    ...
    def get_required_documents(self) -> List[str]:
        return ['deployment_statistics', 'historical_capacity_data', 'policy_history']
