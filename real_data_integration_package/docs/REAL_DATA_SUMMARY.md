...
- ✅ `data/files/ecr_rating_Germany.csv` - Sample ECR data
- ✅ `data/files/ecr_rating_USA.csv` - Sample ECR data
...
### Example 1: Country Stability Agent
```python
    ...
    def _fetch_data(self, country, period):
        ...
        return {
            ...
            'gdp_growth': self.data_service.get_value(
                country, 'gdp_growth', default=0.0
            )
        }
```

### Example 2: Direct Data Service Usage
```python
# Get single value
...
if response.success:
    for point in response.data.data_points:
        print(f"{point.timestamp.year}: {point.value}%")

# Get latest point
latest = data_service.get_latest("Germany", "gdp_growth")
print(f"Latest: {latest.value}% ({latest.timestamp.year})")
```
...
echo "date,value
2024,0.8
2023,0.9" > data/files/ecr_rating_Germany.csv
...
src/data/
├── base/
│   ├── data_types.py          (166 lines)
│   ├── data_models.py         (120 lines)
│   ├── data_source.py         (145 lines)
│   ├── exceptions.py          (35 lines)
│   └── __init__.py            (25 lines)
├── providers/
│   ├── world_bank_provider.py (270 lines)
│   ├── file_provider.py       (240 lines)
│   └── __init__.py            (8 lines)
├── services/
│   ├── data_service.py        (280 lines)
│   ├── cache_manager.py       (245 lines)
│   └── __init__.py            (4 lines)
...
### Configuration:
```
config/data_sources.yaml       (70 lines)
```

### Documentation:
```
ARCHITECTURE.md                (400 lines)
real_data_integration_example.py (300 lines)
quick_start_real_data.py       (200 lines)
...
### Sample Data:
```
data/files/ecr_rating_Germany.csv
data/files/ecr_rating_USA.csv
```
