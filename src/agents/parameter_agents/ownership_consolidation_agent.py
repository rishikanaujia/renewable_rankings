...
try:
    ...
    from research_integration.parsers import OwnershipConsolidationParser
...
class OwnershipConsolidationAgent(*_base_classes):
    ...
    def __init__(
        ...
        if MEMORY_AVAILABLE:
            self.init_memory()
            logger.debug(f"Memory capabilities initialized for OwnershipConsolidationAgent")

        # Configure research parser if available
        if RESEARCH_INTEGRATION_AVAILABLE and OwnershipConsolidationParser:
            self.research_parser = OwnershipConsolidationParser()
            logger.debug(f"Research parser configured for OwnershipConsolidationAgent")
