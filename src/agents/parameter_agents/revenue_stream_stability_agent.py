...
try:
    ...
    from research_integration.parsers import RevenueStreamStabilityParser
...
class RevenueStreamStabilityAgent(*_base_classes):
    ...
    def __init__(
        ...
        if MEMORY_AVAILABLE:
            self.init_memory()
            logger.debug("Memory capabilities initialized for RevenueStreamStabilityAgent")

        # Configure research parser if available
        if RESEARCH_INTEGRATION_AVAILABLE and RevenueStreamStabilityParser:
            self.research_parser = RevenueStreamStabilityParser()
            logger.debug("Research parser configured for RevenueStreamStabilityAgent")
