...
try:
    ...
    from research_integration.parsers import StatusOfGridParser
...
class StatusOfGridAgent(*_base_classes):
    ...
    def __init__(
        ...
        if MEMORY_AVAILABLE:
            self.init_memory()
            logger.debug("Memory capabilities initialized for StatusOfGridAgent")

        # Configure research parser if available
        if RESEARCH_INTEGRATION_AVAILABLE and StatusOfGridParser:
            self.research_parser = StatusOfGridParser()
            logger.debug("Research parser configured for StatusOfGridAgent")
