...
try:
    ...
    from research_integration.parsers import TrackRecordParser
...
class TrackRecordAgent(*_base_classes):
    ...
    def __init__(
        ...
        if MEMORY_AVAILABLE:
            self.init_memory()
            logger.debug("Memory capabilities initialized for TrackRecordAgent")

        # Configure research parser if available
        if RESEARCH_INTEGRATION_AVAILABLE and TrackRecordParser:
            self.research_parser = TrackRecordParser()
            logger.debug("Research parser configured for TrackRecordAgent")
    ...
    def _get_data_sources(self, country: str, data: Dict[str, Any] = None) -> List[str]:
        ...
        elif data and data.get('source') == 'research':
            sources.append("Research System - Track Record Analysis")
            sources.append("IRENA Reports and Industry Data")
